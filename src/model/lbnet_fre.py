...
class EffAttention(nn.Module):
    def __init__(self, args, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
    ...
    def forward(self, x):
        ...
        #         if d in ['Urban100', 'Manga109']:
        #             q_all = torch.split(q, math.ceil(N//32), dim=-2)
        #             k_all = torch.split(k, math.ceil(N//32), dim=-2)
        #             v_all = torch.split(v, math.ceil(N//32), dim=-2)    
        #         else:
        #             q_all = torch.split(q, math.ceil(N//8), dim=-2)
        #             k_all = torch.split(k, math.ceil(N//8), dim=-2)
        #             v_all = torch.split(v, math.ceil(N//8), dim=-2)    

        q_all = torch.split(q, math.ceil(N//64), dim=-2)
        k_all = torch.split(k, math.ceil(N//64), dim=-2)
        v_all = torch.split(v, math.ceil(N//64), dim=-2) 
...
class TransBlock(nn.Module):
    def __init__(
        self, args, n_feat = 64,dim=64, num_heads=8, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.ReLU, norm_layer=nn.LayerNorm):
        super(TransBlock, self).__init__()
        self.dim = dim
        self.atten = EffAttention(args, self.dim, num_heads=num_heads, qkv_bias=False, qk_scale=None, \
...
class FRDAB(nn.Module):
    def __init__(self, n_feats=32):
        ...
        self.fre = common.Frequency_Convolution(n_feats, 8)
...
class LBNet(nn.Module):
    def __init__(self, args, conv=common.default_conv, norm_layer=nn.LayerNorm):
        ...
        kernel_size = 3
        scale = args.scale[0]
        n_feat = 32 #args.n_feats
        num_head = 8 #args.num_heads
        ...
        self.attention = TransBlock(args, n_feat=n_feat, dim=n_feat*9, num_heads = num_head)
        self.attention2 = TransBlock(args, n_feat=n_feat, dim=n_feat*9, num_heads = num_head)
